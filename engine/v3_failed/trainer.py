...
class ReidSystem():
    ...
    def aligned_loss_fns(self, outputs, labels):
        ...
        """
        ce_loss = self.ce_loss(outputs[0], labels)
        global_triplet_loss, local_triplet_losses = self.aligned_triplet(outputs[1], labels, outputs[2])
        #return {'ce_loss': ce_loss, 'globaltriplet': triplet_loss}

        loss_dict = {'ce_loss': ce_loss, 'global_triplet_loss': global_triplet_loss}
        for i, local_loss in enumerate(local_triplet_losses):
            #print('local_loss%d' % i, local_loss)
            loss_dict['local_triplet_loss%d' % i] = local_loss
        return loss_dict
    ...
    def training_step(self, batch):
        ...
        if self.base_type in [BASE_ALIGNED_RESNET50, BASE_ALIGNED_RESNET101, BASE_ALIGNED_RESNEXT101,
        ...
        if (self.global_step+1) % self.log_interval == 0:
            ...
            if 'triplet' in loss_dict.keys():
                ...
                if 'local_triplet_loss0' in loss_dict.keys() :
                    self.writer.add_scalar('local_triplet_loss0', loss_dict['local_triplet_loss0'], self.global_step)
                    self.writer.add_scalar('local_triplet_loss1', loss_dict['local_triplet_loss1'], self.global_step)
                else:
