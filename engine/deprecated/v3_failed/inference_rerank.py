        ...
        for theta in np.linspace(0.5, 1.0, 11):
            for theta2 in np.linspace(0.75, 1.0, 11):
                distmat = re_ranking_v3(qf, gf, k1=6, k2=2, lambda_value=0.3, local_distmat=local_distmat, theta_value=theta,
                                     local_distmat2=local_distmat2, theta_value2=theta2, only_local=False)

                distmat_flipped = re_ranking_v3(qf_flipped, gf_flipped, k1=6, k2=2, lambda_value=0.3, local_distmat=local_distmat_flipped, theta_value=theta,
                                                local_distmat2=local_distmat2_flipped, theta_value2=1.0,
                                                only_local=False)
                ...
                cmc, mAP = evaluate(distmat, q_pids, g_pids, q_camids, g_camids)
                score = (cmc[0] + mAP) / 2
                print('theta, theta2', np.around(theta, 2), np.around(theta2, 3), 'r1, mAP, score', np.around(cmc[0], 4), np.around(mAP, 4),
                      np.around(score, 4))
                if score > best_score:
                    best_score = score
                ...
                if False and abs(theta - 0.95) < 1e-4:
                    # saving dist_mats
                    f = h5py.File('dist_mats/val_%s_%s_t0.95_flip.h5' % (cfg.MODEL.NAME, strtime), 'w')
