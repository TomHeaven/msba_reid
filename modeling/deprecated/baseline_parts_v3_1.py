...
class Baseline(nn.Module):
    ...
    def __init__(self, 
        ...
        self.classifier_final = nn.Linear(self.in_planes + 512 * self.use_parts, self.num_classes, bias=False)
        self.classifier_final.apply(weights_init_classifier)
    ...
    def forward(self, x, label=None):
        ...
        for i in range(self.use_parts):
            ...
            x_ent = self.local_classifiers[i](x_feat)
            lf_xent.append(x_ent)
            if self.training:
                local_feat.append(x_out_feat)
            else:
        ...
        if self.training:
            ...
            # return (cls_score, inv_score, cls_score_br1, cls_score_br2), global_feat
            # lf_xent.extend([cls_score, inv_score, cls_score_br1, cls_score_br2])
           
            local_feat.append(global_feat)
            final_cls_score = self.classifier_final(torch.cat(local_feat, dim=1))

            weights = np.asarray([#1.0, 1.0, # local features
                                1.0, 1.0, # final + global
                                0.03, 0.03     # multi-scale
                                ])
            lf_xent.extend([final_cls_score, cls_score, cls_score_br1, cls_score_br2]*weights)

            return tuple(lf_xent), tuple(local_feat)
        else:
            # return feat
