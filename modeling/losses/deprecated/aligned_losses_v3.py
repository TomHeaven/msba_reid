...
from ..backbones.aligned.local_dist import *
...
class TripletLossAlignedReID(nn.Module):
    ...
    def forward(self, inputs, targets, local_features):
        ...
        ### add support for multiple local features
        ...
        for lf in local_features:
            lf = lf.permute(0,2,1)
            p_local_features = lf[p_inds]
            n_local_features = lf[n_inds]
            local_dist_ap = batch_local_dist(lf, p_local_features)
            local_dist_an = batch_local_dist(lf, n_local_features)
