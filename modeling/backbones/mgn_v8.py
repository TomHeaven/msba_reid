#######
# 思想：二分支简化的mgn，triplet约束合并特征；ce loss约束子特征
######
import copy
import torch
...
class MGN(nn.Module):
    def __init__(self, model_path, num_classes, basename, last_stride, with_ibn, gcb, stage_with_gcb, with_abd, feats = 256, **kwargs):
        ...
        self.classfier0 = nn.Linear(feature_num , num_classes, bias=False)
        self.classfier1_0 = nn.Linear(reduction_num , num_classes, bias=False)
        self.classfier1_1 = nn.Linear(reduction_num , num_classes, bias=False)
    ...
    def forward(self, x):
        ...
        predict = torch.cat([fea0, fea1_0, fea1_1], dim=1) # bs x 4096

        if DEBUG:
            print('predict', predict.size()) # bs x 2048

        if self.training:
            cls0 = self.classfier0(fea0)
            cls1_0 = self.classfier1_0(fea1_0)
            cls1_1 = self.classfier1_1(fea1_1)
