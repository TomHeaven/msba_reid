...
class MGN(nn.Module):
    ...
    def forward(self, x):
        ...
        fg_p1 = self.reduction(zg_p1).squeeze(dim=3).squeeze(dim=2) # bs x 256
        fg_p2 = self.reduction(zg_p2).squeeze(dim=3).squeeze(dim=2)
        fg_p3 = self.reduction(zg_p3).squeeze(dim=3).squeeze(dim=2)
        f0_p2 = self.reduction(z0_p2).squeeze(dim=3).squeeze(dim=2) # bs x 256
        f1_p2 = self.reduction(z1_p2).squeeze(dim=3).squeeze(dim=2)
        f0_p3 = self.reduction(z0_p3).squeeze(dim=3).squeeze(dim=2) # bs x 256
