...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate, ibn):
        ...
        if ibn:
            self.add_module('norm1', BNIN(num_input_features)),
        else:
...
class DenseNet(nn.Module):
    ...
    def load_pretrain(self, model_path=''):
        ...
        for k in state_dict:
            ...
            for i in range(4):
                new_k = new_k.replace('norm.%d' % i, 'norm%d' % i)
                new_k = new_k.replace('conv.%d' % i, 'conv%d' % i)
                new_k = new_k.replace('relu.%d' % i, 'relu%d' % i)
                new_k = new_k.replace('pool.%d' % i, 'pool%d' % i)
            if new_k in self.state_dict() and self.state_dict()[new_k].shape == state_dict[k].shape:
