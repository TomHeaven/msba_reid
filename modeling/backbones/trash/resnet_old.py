...
from ops import *
from modeling.backbones.components.shallow_cam import ShallowCAM
...
class ResNet(nn.Module):
    def __init__(self, last_stride, with_ibn, gcb, stage_with_gcb, block, layers, with_abd=False, with_mpncov=False):
        scale = 64
        self.inplanes = scale
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.with_abd and self.training:
            x_intermediate = self.shallow_cam(x)
    ...
    @classmethod
    def from_name(cls, model_name, last_stride, with_ibn, gcb, stage_with_gcb, with_abd, with_mpncov):
        cls._model_name = model_name
        return ResNet(last_stride, with_ibn, gcb, stage_with_gcb, with_abd=with_abd, with_mpncov= with_mpncov,
