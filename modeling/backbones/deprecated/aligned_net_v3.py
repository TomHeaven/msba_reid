...
from .aligned.HorizontalMaxPool2D import HorizontalMaxPool2d, VerticalMaxPool2d
...
class Aligned_Net(nn.Module):
    expansion = 4
    vertical_aligned = True # False为v2，True为v3
    ...
    def __init__(self, basename, last_stride, with_ibn, gcb, stage_with_gcb, aligned=False, with_abd=False,
        ...
        self.vertical_pool = VerticalMaxPool2d()
        if self.aligned:
            ...
            ### vertical
            if self.vertical_aligned:
                self.vbn = nn.BatchNorm2d(self.feat_dim)
                self.vbn.apply(weights_init_kaiming)  # Tom added
                self.relu = nn.ReLU(inplace=True)
                self.conv2 = nn.Conv2d(self.feat_dim, 256, kernel_size=1, stride=1, padding=0, bias=True)
                self.conv2.apply(weights_init_kaiming)  # Tom added
    ...
    def forward(self, x):
        ...
        if self.aligned:
            ...
            if self.vertical_aligned:
                vlf = self.vbn(x)
                vlf = self.relu(vlf)
                vlf = self.vertical_pool(vlf)
                vlf = self.conv2(vlf)
        if self.aligned or not self.training:
            #print('0 lf', lf.size())  # 0 lf torch.Size([31, 128, 16, 1])
            lf = lf.view(lf.size()[0:3])
            #print('1 lf', lf.size())  # 1 lf torch.Size([31, 128, 16])
            lf = lf / torch.pow(lf, 2).sum(dim=1, keepdim=True).clamp(min=1e-12).sqrt()
            ###
            if self.vertical_aligned:
                #print('0 vlf', vlf.size()) # 0 vlf torch.Size([31, 256, 1, 8])
                vlf = vlf.squeeze()
                #print('1 vlf', vlf.size()) # 1 vlf torch.Size([31, 256, 8])
                vlf = vlf / torch.pow(vlf, 2).sum(dim=1, keepdim=True).clamp(min=1e-12).sqrt()

        if self.aligned and self.vertical_aligned:
            lf = [lf, vlf]
