...
def spp_vertical(feats, pool_list, conv_list, bn_list, relu_list, fc_list, num_strides, feat_list=[], logits_list=[]):
    for i in range(num_strides):
        pcb_feat = pool_list[i](
            feats[:, :, i * int(feats.size(2) / num_strides): (i + 1) * int(feats.size(2) / num_strides), :])
...
class HPM(nn.Module):
    def __init__(self, num_classes, num_stripes=6, local_conv_out_channels=256, erase=0, loss={'xent'}, avg=False,
                 **kwargs):
