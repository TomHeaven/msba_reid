...
class MGN(nn.Module):
    def __init__(self, model_path, num_classes, basename, last_stride, with_ibn, gcb, stage_with_gcb, with_abd, feats = 256, **kwargs):
        ...
        feature_num = 2048
        self.sub_feature = 4
        reduction_num = feature_num // self.sub_feature

        self.reduction1_0 = get_reduction(feature_num, reduction_num)
        self.reduction1_1 = get_reduction(feature_num, reduction_num)
        self.reduction1_2 = get_reduction(feature_num, reduction_num)
        self.reduction1_3 = get_reduction(feature_num, reduction_num)

        self.bottleneck0, self.classifier0 = get_bottleneck_classifier(feature_num, num_classes)
        self.bottleneck1_0, self.classifier1_0 = get_bottleneck_classifier(reduction_num, num_classes)
        self.bottleneck1_1, self.classifier1_1 = get_bottleneck_classifier(reduction_num, num_classes)
        self.bottleneck1_2, self.classifier1_2 = get_bottleneck_classifier(reduction_num, num_classes)
        self.bottleneck1_3, self.classifier1_3 = get_bottleneck_classifier(reduction_num, num_classes)
    ...
    def forward(self, x):
        ...
        p1 = self.p1(x) # output: bs x 2048 x 16 x 8
        p2 = self.p2(x) # output: bs x 2048 x 16 x 8
        ...
        fea1_2 = fea1[:, :, 1:2, :]  # output: bs x 2048 x 1 x 1
        fea1_3 = fea1[:, :, 1:2, :]  # output: bs x 2048 x 1 x 1

        bs = fea0.shape[0]

        fea0 = fea0.view( bs, -1)
        fea1_0 = self.reduction1_0(fea1_0).view(bs, -1) # bs x 512
        fea1_1 = self.reduction1_1(fea1_1).view(bs, -1) # bs x 512
        fea1_2 = self.reduction1_2(fea1_2).view(bs, -1)  # bs x 512
        fea1_3 = self.reduction1_3(fea1_3).view(bs, -1)  # bs x 512

        bn_fea0 = self.bottleneck0(fea0)
        bn_fea1_0 = self.bottleneck1_0(fea1_0)
        bn_fea1_1 = self.bottleneck1_1(fea1_1)
        bn_fea1_2 = self.bottleneck1_2(fea1_2)
        bn_fea1_3 = self.bottleneck1_3(fea1_3)

        predict = torch.cat([bn_fea0, bn_fea1_0, bn_fea1_1, bn_fea1_2, bn_fea1_3], dim=1) # bs x 4096
        ...
        if self.training:
            ...
            cls1_0 = self.classifier1_0(bn_fea1_0)
            cls1_1 = self.classifier1_1(bn_fea1_1)
            cls1_2 = self.classifier1_2(bn_fea1_2)
            cls1_3 = self.classifier1_3(bn_fea1_3)

            return predict, (predict, fea0, fea1_0, fea1_1, fea1_2, fea1_3), (cls0, cls1_0, cls1_1, cls1_2, cls1_3)
        else:
            return predict
