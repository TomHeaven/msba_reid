...
def get_dataloader(cfg, return_sets=False):
    ...
    for d in cfg.DATASETS.NAMES:
        dataset = init_dataset(d, root=cfg.DATASETS.ROOT_DIR, fine_tune=cfg.MODEL.FINE_TUNE)
    ...
    if return_sets:
        return tng_dataloader, val_dataloader, tng_set.c, len(query_names), tng_set, val_set

    return tng_dataloader, val_dataloader, tng_set.c, len(query_names)


def get_test_dataloader(cfg, test_phase):
    val_tfms = build_transforms(cfg, is_train=False)

    print('prepare test set ...')
    dataset = init_dataset(cfg.DATASETS.TEST_NAMES, cfg.DATASETS.ROOT_DIR, test_phase)
    query_names, gallery_names = dataset.query, dataset.gallery
    ...
    test_dataloader = DataLoader(test_set, cfg.TEST.IMS_PER_BATCH, num_workers=num_workers,
                                collate_fn=fast_collate_fn, pin_memory=True)
    return test_dataloader, len(query_names), dataset
