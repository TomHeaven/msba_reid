...
def build_transforms(cfg, is_train=True):
    ...
    if is_train:
        ...
        if cfg.INPUT.DO_PAD:
            ###### Tan ########
            ## add RandomResizeCrop
            res.append(T.RandomChoice(
                [
                    T.Compose(
                        [T.Pad(cfg.INPUT.PADDING, padding_mode=cfg.INPUT.PADDING_MODE),
                         T.RandomCrop(cfg.INPUT.SIZE_TRAIN)]
                    ),
                    T.RandomResizedCrop(tuple(cfg.INPUT.SIZE_TRAIN), scale=(0.8, 1.0), interpolation=Image.BILINEAR)
                ]
            ))
            #################
