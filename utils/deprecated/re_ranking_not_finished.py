...
def re_ranking(original_dist, query_num, gallery_num, k1, k2, lambda_value, local_distmat=None, theta_value=0.5, only_local=False):
    ...
    print('Allocate V ...')
    V_ind = np.zeros_like(initial_rank, dtype=np.int32)
    V = np.zeros_like(initial_rank, dtype=np.float16) # 14 GB memory
    ...
    for i in tqdm.tqdm(range(all_num)): # Time: 18 s
        ...
        if DEBUG:
            print('k_reciprocal_index', k_reciprocal_index)
        ...
        if DEBUG:
            print('k_reciprocal_expansion_index', len(k_reciprocal_expansion_index))
    ...
    if k2 != 1:
        ...
        for i in tqdm.tqdm(range(all_num)): # Time: 12.6 min
            V_qe[i, :] = np.mean(V[initial_rank[i, :k2], :], axis=0)
            V_qe_ind[i,:] = V_ind[initial_rank[i, :k2], :]
    ...
    for i in tqdm.tqdm(range(gallery_num)):   # Time: 20 s
        idx = np.where(V[:, i] != 0)[0]
        #invIndex.append(np.where(V[:, i] != 0)[0])
        invIndex.append(V_ind[idx, i])
