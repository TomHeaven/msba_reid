...
def re_ranking(original_dist, query_num, gallery_num, k1, k2, lambda_value, local_distmat=None, theta_value=0.5, only_local=False):
    ...
    if k2 != 1:
        ...
        V_qe_ind = np.zeros_like(V_ind, dtype=np.int32)
        for i in tqdm.tqdm(range(all_num)):  # Time: 12.6 min
            idx = V_ind[initial_rank[i, :k2], :]

            V_qe[i, idx] = np.mean(V[initial_rank[i, :k2], :], axis=0)
            #print('V_ind', V_ind[initial_rank[i, :k2]])

            candidate = V_ind[initial_rank[i, :k2], :]
            indNonZero = np.where(candidate != -1)
            k_reciprocal_expansion_index = np.unique(candidate[indNonZero])
            V_qe_ind[i, :len(k_reciprocal_expansion_index)] = k_reciprocal_expansion_index
    ...
    #for i in tqdm.tqdm(range(gallery_num)):  # Time: 20 s
        # invIndex.append(np.where(V[:, i] != 0)[0])
    #    invIndex.append(np.where(Vt[i] != -1)[0])
    ########################
    ...
    for i in tqdm.tqdm(range(query_num)):    # Time: 1 min
        ...
        #indImages = [ind for ind in V_ind[indNonZero]]
