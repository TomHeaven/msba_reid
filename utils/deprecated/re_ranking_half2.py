...
def re_ranking(original_dist, query_num, gallery_num, k1, k2, lambda_value, local_distmat=None, theta_value=0.5, only_local=False):
    ...
    V_ind = np.ones((initial_rank.shape[0], initial_rank.shape[1]*3), dtype=np.int32) * -1
    ...
    if k2 != 1:
        ...
        #V_qe_ind = np.ones((initial_rank.shape[0], initial_rank.shape[1] * 2), dtype=np.int32) * -1
        ...
        for i in tqdm.tqdm(range(all_num)):  # Time: 12.6 min
            ...
            idx = np.unique(idx)
            print('idx', idx.shape, 'V_qe', V_qe[i, idx].shape)
            #
            #V_qe_ind[i, :] = idx
            V_qe[i, idx] = np.mean(V[initial_rank[i, :k2], :], axis=0)[:len(idx)]
        V = V_qe
        del V_qe
    del initial_rank
    invIndex = []
