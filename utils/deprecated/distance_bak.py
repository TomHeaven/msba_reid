...
def fast_matrix_op(
  ...
  """
    ...
    mat = np.zeros((x.shape[0], y.shape[0]), dtype=np.float16)

    pi = 0
    for i, part_x in enumerate(np.array_split(x, x_num_splits, axis=x_split_axis)):
        # move to cuda
        part_x = torch.tensor(part_x, dtype=torch.float32).cuda()
        dx = part_x.size()[0]
        pj = 0
        for j, part_y in enumerate(np.array_split(y, y_num_splits, axis=y_split_axis)):
            # move to cuda
            part_y = torch.tensor(part_y, dtype=torch.float32).cuda()
            dy = part_y.size()[0]
            #print('dx, dy', dx, dy)
            # compute using gpu
...
def fast_local_dist(x, y, aligned=True):
    print('Computing local distance...')
    ...
    y_num_splits = int(len(y) / slice_size) + 1
    #y_num_splits = 1
    z = fast_matrix_op(x, y, 0, 0, x_num_splits, y_num_splits, verbose=True)
    return z
