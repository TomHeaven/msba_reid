...
### 这里是减小内存开销的关键 （idx仅需要存储 [all_num, k1+1]，节约一个4 Byte大方阵，减少内存占用 20 GB，并大幅加快推理速度）
...
def compute_distmat_using_gpu(probFea, galFea, memory_save=True, mini_batch=5000):
    ...
    if memory_save:
        distmat = torch.zeros((all_num, all_num), dtype=torch.float16)  # 9.53 GB memory on Round2
        i = 0
        while True:
            it = i + mini_batch
            # print('i, it', i, it)
    ...
    original_dist = distmat.numpy()  # 9.53 GB memory
    del distmat
    del feat
...
def re_ranking(original_dist, query_num, gallery_num, k1, k2, lambda_value, local_distmat=None, theta_value=0.5, only_local=False):
    ...
    #V = lil_matrix(original_dist.shape, dtype=np.float32) # save memory by using linked sparse matrix instead numpy array, but too slow
