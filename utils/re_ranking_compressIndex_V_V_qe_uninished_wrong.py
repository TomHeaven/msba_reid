...
def dense2sparse(dense_mat, col_idx, width):
    #print('dense_mat', dense_mat.shape, 'col_idx', len(col_idx))
    assert dense_mat.ndim == 1
    sparse_mat = np.zeros((1, width), dtype=dense_mat.dtype)
    for j, idx in enumerate(col_idx):
        sparse_mat[0, j] = dense_mat[idx]
    return sparse_mat
...
def re_ranking(original_dist, query_num, gallery_num, k1, k2, lambda_value, local_distmat=None, theta_value=0.5, only_local=False):
    ...
    print('Allocate V ...')
    #V = np.zeros_like(original_dist, dtype=np.float16) # 9.51 GB memory

    # save memory by using V_ind and V instead of original V (9.51 GB)
    V_ind = np.ones((initial_rank.shape[0], initial_rank.shape[1]*10), dtype=np.int32) * -1
    ...
    for i in tqdm.tqdm(range(all_num)): # Time: 18 s
        ...
        #print('k_reciprocal_expansion_index', k_reciprocal_expansion_index)
        #print('V_ind[i]', V_ind[i])
    ...
    if k2 != 1:
        V_qe = np.zeros_like(V_ind, dtype=np.float16)
        V_qe_ind = np.ones_like(V_ind, dtype=np.int32) * -1
        for i in tqdm.tqdm(range(all_num)):  # Time: 12.6 min
            ...
            print('idx', idx)
            V_qe_ind[i, :len(idx)] = idx
            V_qe[i, :] = dense2sparse(new_V, idx, V_ind.shape[1])
    ...
    # Vt （V的转置矩阵）的计算可能有问题
    counter = np.zeros(V_ind.shape[0], dtype=np.int32)
    Vt = np.zeros_like(V_ind, dtype=np.float16)
    Vt_ind = np.ones_like(V_ind, dtype=np.int32) * -1
    for i in tqdm.tqdm(range(all_num)): # 扫描行
        for j, idx in enumerate(V_ind[i]):
            if idx != -1:
                p = counter[idx]
                Vt[idx][p] = V[i][j]
                Vt_ind[idx][p] = i
    ...
    for i in tqdm.tqdm(range(gallery_num)):  # Time: 20 s
        invIndex.append(np.where(Vt[i] != 0)[0])
    ########################
    ...
    for i in tqdm.tqdm(range(query_num)):    # Time: 1 min
        temp_min = np.zeros(shape=[1, gallery_num], dtype=np.float16)
        #indNonZero = np.where(V_ind[i] != -1)[0]
        indNonZero = np.where(V[i, :] != 0)[0]

        indImages = [invIndex[ind] for ind in V_ind[i, indNonZero]]
        #indImages = V_ind[i][indNonZero]
        print('indNonZero', len(indNonZero), indNonZero, 'ind', V_ind[i, indNonZero], 'indImages', len(indImages), indImages)
        for j in range(len(indNonZero)):
            temp_min[0, indImages[j]] = temp_min[0, indImages[j]] + np.minimum(V[i, indNonZero[j]],
                                                                               Vt[indImages[j], indNonZero[j]])
        #######
        jaccard_dist_i = 1 - temp_min / (2 - temp_min)
        original_dist[i] = jaccard_dist_i * (1 - lambda_value) + original_dist[i] * lambda_value
        #######
